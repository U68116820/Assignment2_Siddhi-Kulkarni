...
namespace Assignment2_Siddhi_Kulkarni
{
    class Program
    {
        ...
        private static void RestoreString(string s5, int[] indices)
        {
            try
            {
                char[] a = new char[s5.Length];
                char[] b = new char[s5.Length];
                ...
                while (j < s5.Length && i < indices.Length)
                {
                    int k = indices[i];
                    b[k] = a[j];
                    j += 1;
            ...
            */
            {
                ...
                {
                    // If current character of str61 is  
                    // seen first time in it. 
                    if (map[s61[i]] == -1)
                    {

                        // If current character of str62 
                        // is already seen, one to 
                        // one mapping not possible 
                        if (marked[s62[i]] == true)
                            return false;
       ...
       */
            {
                ...
                for (int i = 0; i < 11; i++)
                {
                    ...
                    {
                        map[scores[i, 0]].Add(scores[i, 1]);
                    }
                    else
                    {
                        map.Add(scores[i, 0], new List<int> { scores[i, 1] });
                    }
                }
           ...
           */
            {
                ...
                {
                    for (int j = i; j < ar9.Length; j++)
                    {
                        if (ar9[j] > ar9[i])
                        {
                            x = ar9[j] - ar9[i];
                            if (x > diffrence) { diffrence = x; }
                        }
                    }
                }
                return diffrence;
        ...
        /// </summary>
        static int fib(int n10)
        {
            if (n10 <= 1)
                return n10;
            return fib(n10 - 1) + fib(n10 - 2);
        }
